...
void nhapPhieu(phieuBan& x) {
	...
	nhapDsVL(x.dsvl);
	cout << "\ Nhap so tien dat coc: "; cin >> x.DatCoc;
...
void inPhieu(phieuBan x) {
	...
	inKH(x.kh);
	inDSVL(x.dsvl);
	cout << "\n Tong tien: " <<(size_t)TongTienVL(x.dsvl);
	cout << "\n So tien dat coc: " << x.DatCoc;
}
...
void TongTienDatCoc(listPhieuBan l) {
	float tienDC = 0;
	for (nodePhieuBan* k = l.pHead; k != NULL; k = k->pNext) {
		tienDC += k->pb.DatCoc;
	}
	cout << "\n Tong tien da dat coc: " << (size_t)tienDC;
}
float timkiemPhieuCoTienDatCocMax(listPhieuBan l) {
	float max = l.pHead->pb.DatCoc;
	for (nodePhieuBan* k = l.pHead->pNext; k != NULL; k = k->pNext) {
		if (k->pb.DatCoc > max) {
			max = k->pb.DatCoc;
		}
...
void PhieuCoTienDatCocMax(listPhieuBan l) {
	for (nodePhieuBan* k = l.pHead; k != NULL; k = k->pNext) {
		if (k->pb.DatCoc == timkiemPhieuCoTienDatCocMax(l)) {
			inPhieu(k->pb);
		}
	}
}
// void timkiemPhieuCoTienDatCocMax(listPhieuBan l) {
	//	float max = l.pHead->pb.DatCoc;
	//	for (nodePhieuBan* k = l.pHead->pNext; k != NULL; k = k->pNext) {
	//		if (k->pb.DatCoc > max) {
	//			max = k->pb.DatCoc;
...
bool kiemTraSoi(listVl l) {
	string tenvl = "soi";
	for (nodeVl* k = l.pHead; k != NULL; k = k->pNext) {
		string ten = k->vl.tenVl;
		if (strcmp((char*)ten.c_str(), (char*)tenvl.c_str())==0) {
			return true;
		}
	}
	return false;
}
void tinhTienTBCongPhieuChuaSoi(listPhieuBan l) {
	int tien = 0, n=0;
	for (nodePhieuBan* k = l.pHead; k != NULL; k = k->pNext) {
		if (kiemTraSoi(k->pb.dsvl)==true) {
			tien += k->pb.tongTien;
			n++;
		}
	}
	cout << "\n Trung binh cong: " << tien / n;
}
...
//
//void ghiFilePB(listPhieuBan L)
//{
//
//	FILE* f = fopen("PhieuBan.dat", "w+b");
//	for (nodePhieuBan* p = L.pHead; p != NULL; p = p->pNext)
//	{
...
//}
//void docFilePB(listPhieuBan& L)
//{
//	FILE* f = fopen("PhieuBan.dat", "r+b");
//	phieuBan x;
//	nodePhieuBan* p;
//	khoiTaoListPhieu(L);
...
{
	fstream f;
	f.open("PhieuBanLKDon.dat", ios::out|ios::binary);
	for (nodePhieuBan* p = l.pHead; p != NULL; p = p->pNext) {
...
void docFilePB(listPhieuBan& l)
{
	...
	phieuBan x;
	khoiTaoListPhieu(l);
	f.open("PhieuBanLKDon.dat", ios::in|ios::binary);
...
void menu(listPhieuBan& l) {
	...
	while (true) {
		...
		cout << "\n\t************19. Tong tien dat coc ";
		cout << "\n\t*********** 20. Phieu ban co tien dat coc nhieu nhat";
		cout << "\n\t 21: Trung binh cong ";
		cout << "\n Lua chon cua ban: ";
		cin >> chon;
		if (chon == 1) {
			...
			PhieuCoTienDatCocMax(l);
			system("pause");
		}
		else if (chon == 21) {
			tinhTienTBCongPhieuChuaSoi(l);
			system("pause");
		}
		else if (chon == 0) {
			break;
